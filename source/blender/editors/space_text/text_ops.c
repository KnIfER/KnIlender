...
static int text_expand_region_width_poll(bContext *C)
{
	return 1;
}

static int text_expand_region_width_exec(bContext *C, wmOperator *op)
{
	expand_region_to_right_exec(C);
}

void TEXT_OT_expand_region_width(wmOperatorType *ot)
{
	/* identifiers */
	ot->name = "Expand Region Width";
	ot->idname = "TEXT_OT_expand_region_width";
	ot->description = "Expand Region Width horizontally";
	
	/* api callbacks */
	ot->poll = text_expand_region_width_poll;
	ot->exec = text_expand_region_width_exec;
